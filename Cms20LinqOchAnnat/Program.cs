...
namespace Cms20LinqOchAnnat
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var over30GoalsLinq = players.Count(p => p.Goals > 30);
            ...
            var myList = players.Where(p => p.Team == team).OrderByDescending(p => p.Name).ToList();

            // 6. Select all players from Team (chosen by user) - Writing player Name to console in a FOREACH loop
            foreach (var p in players.Where(p => p.Team == team).OrderByDescending(p => p.Name))
            {
